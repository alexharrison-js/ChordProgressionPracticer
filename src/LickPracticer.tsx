...
export default function LickPracticer({
  ...
  // Sound source — WJD-only for now (Bopland lick playback, if any, is
  // handled inside BoplandLickView itself). "synth" preserves the original
  // oscillator-based timbre picker; "sample" plays through real recorded
  // instrument samples instead (see musicEngine.ts / SoundSourceSelector).
  ...
  async function playLick() {
    ...
    // Safety net: covers a lick reaching outside the eagerly-preloaded
    // range, or playback starting before that preload finished.
    if (soundSource.kind === "sample") {
      const midiNotes = currentLick.notes
        .filter((n) => !n.rest && n.midi !== undefined)
        .map((n) => n.midi!);
      await engine.preloadSamples(soundSource.instrumentId, midiNotes);
    }
    ...
    currentLick.notes.forEach((n) => {
      ...
      if (!n.rest && n.midi !== undefined) {
        engine.playNote(n.midi, t, beats * secPerBeat * 0.9, soundSource);
      }
